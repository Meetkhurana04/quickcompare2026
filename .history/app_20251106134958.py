...
def merge_shop_data(query, comparify_data, shop_data):
    ...
    for source, data in comparify_data.items():
        ...
        for product in products:
            ...
            image_url = product.get('image_url') or product.get('imageUrl') or product.get('productVariant', {}).get('images', [{}])[0].get('path', '')
            if image_url and not image_url.startswith('http'):
                image_url = 'https://cdn.dmart.in/' + image_url if 'dmart' in image_url.lower() else image_url
            ...
            if 'variations' in product and product['variations']:
                ...
                for variation in product['variations']:
                    ...
                    var_image = variation.get('images', [])
                    if var_image and isinstance(var_image, list) and var_image:
                        var_image = var_image[0]
                    else:
                        var_image = image_url
